...
for idx = 1:nr_TEs
    gre_data_fullpaths{idx}  = fullfile(data_directory, 'h5'   , sprintf('%s.h5', gre_filenames{idx}));
    gre_noise_fullpaths{idx} = fullfile(data_directory, 'noise', sprintf('noise_%s.h5', gre_filenames{idx}));
    gre_dat_fullpaths{idx}   = fullfile(data_directory, sprintf('%s.dat', gre_filenames{idx}));
end
...
for idx = 9%:nr_recons
    ...
    [im_king,im_fs] = perform_deblurring_king_method(double(kspace), nufft_st, w, csm, gx_predicted, gy_predicted, gz_predicted, x, y, z, rotMatrixRCSToGCS, rotMatrixGCSToPCS, rotMatrixPCSToDCS, field_of_view_mm, DCS_offset, gamma, B0, dt);
    fprintf('done! (%6.4f/%6.4f sec)\n', toc, toc(start_time));
    ...
    tic; fprintf('Saving results: %s... ', output_fullpath);
    save(output_fullpath, 'im_maxgirf_lowrank');
    fprintf('done! (%6.4f sec)\n', toc);
    ...
    figure('Color', 'w');
    imagesc(flip(im_cartesian(:,:,actual_slice_nr),2)); axis image off;
    colormap(gray(256));
    ...
    export_fig(fullfile(output_directory, sprintf('B0map_slice%d', actual_slice_nr)), '-r400', '-tif', '-c[50,400,200,470]');
    colorbar; caxis([-60 60]);
    export_fig(fullfile(output_directory, sprintf('B0map_slice%d_with_colorbar', actual_slice_nr)), '-r400', '-tif');
end
...
N1_zoom = 260; 
N2_zoom = 285;
idx1_zoom = (-floor(N1_zoom/2):ceil(N1_zoom/2)-1).' + floor(N1_zoom/2) + 1 + 30;
idx2_zoom = (-floor(N2_zoom/2):ceil(N2_zoom/2)-1).' + floor(N2_zoom/2) + 1 + 30;

im1 = reorient(im_cartesian(idx1_zoom,idx2_zoom,actual_slice_nr));
im2 = reorient(abs(im_king(idx1_zoom,idx2_zoom)));
im3 =  reorient(abs(im_maxgirf_lowrank(idx1_zoom,idx2_zoom)));
im1 = im1 / max(abs(im1(:)));
im2 = im2 / max(abs(im2(:)));
im3 = im3 / max(abs(im3(:)));
cmax = 0.67;
...
ax1 = subplot(1,3,1);
imagesc(ax1, abs(im1)); axis image off;
colormap(ax1, gray(256));
title('Cartesian reference (SE)', 'Color', color_order(3,:), 'FontSize', 20);
caxis([0 cmax]);

ax2 = subplot(1,3,2);
imagesc(ax2, abs(im2)); axis image off;
colormap(ax2, gray(256));
title('King''s method', 'Color', color_order(3,:), 'FontSize', 20);
caxis([0 cmax]);

ax3 = subplot(1,3,3);
imagesc(ax3, abs(im3)); axis image off;
colormap(ax3, gray(256));
title('MaxGIRF + lowrank (CG)', 'Color', color_order(3,:), 'FontSize', 20);
caxis([0 cmax]);

pos1 = get(ax1, 'Position');
pos2 = get(ax2, 'Position');
pos3 = get(ax3, 'Position');
set(ax1, 'Position', [0.0000 0.0100 0.4000 0.6500]); % [left bottom width height]
set(ax2, 'Position', [0.3000 0.0100 0.4000 0.6500]); % [left bottom width height]
set(ax3, 'Position', [0.6000 0.0100 0.4000 0.6500]); % [left bottom width height]
export_fig(fullfile(output_directory, sprintf('cartesian_vs_king_vs_MaxGIRF_slice%d', idx)), '-r400', '-tif');

%% zoom-in

N1_zoom = 220; 
N2_zoom = 150;
idx1_zoom = (-floor(N1_zoom/2):ceil(N1_zoom/2)-1).' + floor(N1/2) + 1 - 5;
idx2_zoom = (-floor(N2_zoom/2):ceil(N2_zoom/2)-1).' + floor(N2/2) + 1 - 60;
...
im1 = reorient(im_cartesian(idx1_zoom,idx2_zoom,actual_slice_nr));
im2 = reorient(im_king(idx1_zoom,idx2_zoom));
im3 =  reorient(im_maxgirf_lowrank(idx1_zoom,idx2_zoom));
im1 = im1 / max(abs(im1(:)));
im2 = im2 / max(abs(im2(:)));
im3 = im3 / max(abs(im3(:)));
cmax = 1.1;

h = figure('Color', 'k', 'Position', [1 1 1600 823]);
filename = 'king_vs_maxgirf3.gif';

imagesc(abs(cat(2, im1, im2))); axis image off;
colormap(gray(256));
caxis([0 cmax]);
text(N1_zoom/2, 0, 'Cartesian reference (SE)', 'Color', color_order(3,:), 'FontSize', 20, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center');
text(N1_zoom/2+N1_zoom, 0, 'NUFFT', 'Color', color_order(3,:), 'FontSize', 20, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center');
frame = getframe(h);
im = frame2im(frame);
[imind,cm] = rgb2ind(im,256);
imwrite(imind, cm, filename, 'gif', 'Loopcount', inf);

imagesc(abs(cat(2, im1, im3))); axis image off;
text(N1_zoom/2, 0, 'Cartesian reference (SE)', 'Color', color_order(3,:), 'FontSize', 20, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center');
text(N1_zoom/2+N1_zoom, 0, 'MaxGIRF + lowrank (CG)', 'Color', color_order(3,:), 'FontSize', 20, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center');
colormap(gray(256));
caxis([0 cmax]);
frame = getframe(h);
im = frame2im(frame);
...
N2_zoom = 280;
idx1_zoom = (-floor(N1_zoom/2):ceil(N1_zoom/2)-1).' + floor(N1/2) + 1 - 5;
idx2_zoom = (-floor(N2_zoom/2):ceil(N2_zoom/2)-1).' + floor(N2/2) + 1 + 10;
...
h = figure('Color', 'k', 'Position', [1 1 1600 823]);
filename = 'SE_vs_king_vs_maxgirf_whole.gif';

imagesc(abs(cat(2, im1, im2))); axis image off;
colormap(gray(256));
caxis([0 cmax]);
text(N1_zoom/2, 0, 'Cartesian reference (SE)', 'Color', color_order(3,:), 'FontSize', 20, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center');
text(N1_zoom/2+N1_zoom, 0, 'King'' method', 'Color', color_order(3,:), 'FontSize', 20, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center');
...
colormap(hot(256));
hc = colorbar;
set(hc, 'FontSize', 14, 'Color', 'w');
title(sprintf('Off-resonance map [Hz]'), 'Color', 'w', 'FontSize', 14);
export_fig(fullfile(output_directory, sprintf('B0map_slice%d_bbg', actual_slice_nr)), '-r400', '-tif');




%% Make a movie for the human axial scan
figure('Color', 'k', 'Position', [1 1 1600 823]);
color_order = get(gca, 'colororder');
...
%vidfile = VideoWriter(sprintf('king_vs_MaxGIRF_slice%d.mp4', idx), 'MPEG-4');
...
for idx1 = 1:2
    ax1 = subplot(1,3,1);
    imagesc(ax1, im1); axis image off;
    caxis([0 cmax]);
    colormap(ax1, gray(256));
    title('Cartesian reference (SE)', 'Color', color_order(3,:), 'FontSize', 20);
    ...
    ax2 = subplot(1,3,2);
    if idx1 == 1
        imagesc(ax2, im2); axis image off;
        title_text = 'NUFFT';
    else
        imagesc(ax2, im3); axis image off;
        title_text = 'MaxGIRF + lowrank (CG)';
    end
    caxis([0 cmax]);
    colormap(ax2, gray(256));
    title(title_text, 'Color', color_order(3,:), 'FontSize', 20);

    ax3 = subplot(1,3,3);
    imagesc(reorient(B0map(idx1_zoom,idx2_zoom,actual_slice_nr))); axis image off;
    title('Static off-resonance map [Hz]', 'FontSize', 14, 'Color', 'w');
    ...
    pos2 = get(ax2, 'Position');
    pos3 = get(ax3, 'Position');
    set(ax1, 'Position', [0.0000 0.0100 0.4000 0.8000]); % [left bottom width height]
    set(ax2, 'Position', [0.3600 0.0100 0.4000 0.8000]); % [left bottom width height]
    set(ax3, 'Position', [0.7400 0.0100 0.2134 0.8150]); % [left bottom width height]

    if idx1 == 1
        export_fig('image_nufft', '-r864', '-tif');
    else
        export_fig('image_maxgirf', '-r864', '-tif');
    end
    %frame = getframe(gcf);
...
% im4 = flip(rot90(abs(im_cpr(:,:,end)) / max(abs(vec(im_cpr(:,:,end)))),-1),2);
% im5 = flip(rot90(abs(im_lowrank) / max(abs(im_lowrank(:))),-1),2);
% im6 = flip(rot90(abs(im_gre(:,:,1,1)) / max(abs(vec(im_gre(:,:,1,1)))),-1),2);
...
for idx = 1:Ni
    hPlot1 = plot(ax1, t*1e3, gx_predicted(:,idx)*1e1, 'LineWidth', 2, 'Color', color_order(1,:));
    hPlot2 = plot(ax1, t*1e3, gy_predicted(:,idx)*1e1, 'LineWidth', 2, 'Color', color_order(2,:));
    h_current(1) = line(ax1, t(1)*1e3, abs(gx_predicted(1,idx)*1e1), 'Marker', '.', 'MarkerSize', 20, 'Color', 'b');
    h_current(2) = line(ax1, t(1)*1e3, abs(gy_predicted(1,idx)*1e1), 'Marker', '.', 'MarkerSize', 20, 'Color', 'r');
    ...
    title(ax1, {'GIRF-predicted phyiscal gradients', sprintf('Interleaf number = %d', idx)}, 'Color', text_color);
    ...
    for idx1 = 1:80:Nk
        ...
        set(h_current(1), 'XData', t(idx1)*1e3, 'YData', gx_predicted(idx1,idx)*1e1);
        set(h_current(2), 'XData', t(idx1)*1e3, 'YData', gy_predicted(idx1,idx)*1e1);

        imagesc(ax2, reshape(KP(idx1,:,idx)*0, [N1 N2]));
        axis(ax2, 'image', 'off');
        ...
        imagesc(ax3, reshape(KP(idx1,:,idx) / (2 * pi * t(end)), [N1 N2]));
        axis(ax3, 'image', 'off');
...
figure('Color', 'w');
for idx = 1:maxiter
    imagesc(flip(rot90(abs(reshape(im3(:,idx), [N1 N2])),-1),2)); axis image;
    colormap(gray(256));
    colorbar;
    title(sprintf('Iteration number = %d', idx));
    pause;
end
%%
figure('Color', 'w');
