function outp = encoding_lowrank_maxgirf_multi_gpu(inp, csm_device, U_device, V_device, w_device, st_device, transpose_indicator)
...
%% Define dimensions
[N1,N2,Nc] = size(csm_device{1});
N = N1 * N2; % number of spatial locations in a 2D grid
[Nk,L,Ni] = size(U_device{1});
nr_GPUs = length(csm_device);
...
spmd (nr_GPUs)
    outp_device = complex(zeros(N, 1, 'double', 'gpuArray'));
    start_time = tic;
    for i = 1:Ni
        Nd = st_device{i}.Nd;
