%function [im_nufft_multislice,header,r_dcs_multislice] = siemens_gridding_recon_delay_recon(ismrmrd_noise_fullpath, ismrmrd_data_fullpath, siemens_dat_fullpath, user_opts)
...
% A minus sign on k-space trajectory is used to change from exp(-k(t) * r) to exp(-(-k(t)) * r)
...
for idx = 3%1:nr_recons
    ...
    imc_bart = complex(zeros(N1, N2, Nc, 'double'));
    for c = 1:Nc
        tstart = tic; fprintf('(%2d/%2d): BART NUFFT reconstruction (c=%2d/%2d)... ', idx, nr_recons, c, Nc);
        bart_command = sprintf('bart nufft -i -d %d:%d:%d -l 0.1', N1, N2, N3);
        imc_bart(:,:,c) = bart(bart_command, coords*N1, reshape(kspace(:,:,c), [1 Nk Ni 1]));
        fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));
    end
    
    if 0
    %%
    im_bart_sos = sum(abs(imc_bart).^2, 3);
    figure('Color', 'w', 'Position', [1 1 1600 823]);
    imagesc(im_bart_sos); axis image off; colormap(gray(256));
    title('BART');
    export_fig('bart_nufft_slice3', '-tif');
    %%
    figure('Color', 'w', 'Position', [1 1 1600 823]);
    imagesc(abs(im_nufft_multislice(:,:,idx))); axis image off; colormap(gray(256));
    %title('MIRT');
    export_fig('nufft_girf_slice3', '-tif');
    ...
    %
    g_gcs_nom = zeros(Nk, 3, Ni, 'double');
    g_gcs_nom(:,1,:) = phase_sign * gu_nominal;
    g_gcs_nom(:,2,:) = read_sign  * gv_nominal;
    k_gcs_nom = cumsum(gamma * g_gcs_nom * 1e-2 * double(dt)); % [rad/m]
    k_rcs_nom = zeros(Nk, 3, Ni, 'double');
    for i = 1:Ni
        k_rcs_nom(:,:,i) = (R_rcs2gcs.' * k_gcs_nom(:,:,i).').'; % Nk x 3
    end
    coords = -permute(k_rcs_nom, [2 1 3]) / (2 * krmax * 2 * pi * 1e2); % Nk x 3 x Ni => 3 x Nk x Ni

    data = reshape(kspace, [1 Nk Ni Nc]);
    k_traj = coords;
