...
%% Calculate GIRF-predicted gradient waveforms in RCS [G/cm] [r,c,s]
g_rcs = zeros(Nk, 3, Ni, 'double');
for i = 1:Ni
    g_rcs(:,:,i) = (R_rcs2gcs.' * g_gcs(:,:,i).').'; % Nk x 3
end
...
im_king_multislice = complex(zeros(N1, N2, nr_slices, 'double'));
...
for idx = 1:nr_recons
    ...
    %% Calculate the transformation matrix A from RCS to DCS
    %----------------------------------------------------------------------
    ...
    % [gz]                                       [gs]
    %----------------------------------------------------------------------
    A = R_pcs2dcs * R_gcs2pcs * R_rcs2gcs;
    ...
    %% Calculate the spatial coordinates in XYZ [m]
    XYZ = (A.' * r_dcs.').'; % ((3 x 3) * (N x 3).') => N x 3
    X = reshape(XYZ(:,1), [N1 N2]);
    Y = reshape(XYZ(:,2), [N1 N2]);
    Z = reshape(XYZ(:,3), [N1 N2]);

    %% Calculate the gradients in XYZ [G/cm]
    GX = reshape(g_rcs(:,1,:), [Nk Ni]); % Nk x Ni [G/cm]
    GY = reshape(g_rcs(:,2,:), [Nk Ni]); % Nk x Ni [G/cm]
    GZ = reshape(g_rcs(:,3,:), [Nk Ni]); % Nk x Ni [G/cm]

    %% Perform through-plane correction
    tstart = tic; fprintf('(%2d/%2d): Performing through-plane correction... ', idx, nr_recons);
    %----------------------------------------------------------------------
    ...
    for c = 1:Nc
        tstart = tic; fprintf('(%2d/%2d): NUFFT reconstruction (c=%2d/%2d)... ', idx, nr_recons, c, Nc);
        imc_nufft(:,:,c) = nufft_adj(reshape(double(kspace_demod(:,:,c)) .* w, [Nk*Ni 1]), nufft_st) * scale_factor;
        fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));
    end

    %% IFFT to k-space (k-space <=> image-space)
    kspace_demod_gridded = imc_nufft;
    for dim = 1:2
        kspace_demod_gridded = sqrt(size(kspace_demod_gridded,dim)) * fftshift(ifft(ifftshift(kspace_demod_gridded, dim), [], dim), dim);
    end
    ...
    %----------------------------------------------------------------------
    D = max(encoded_fov) * 1e-3; % [mm] * [m/1e3mm] => [m]
    index = (X.^2 + Y.^2 < (D / 2).^2);
    ...
    %----------------------------------------------------------------------
    im_fs = complex(zeros(N1, N2, L, 'double'));
    for idx1 = 1:L
        tstart = tic; fprintf('(%2d/%2d): Performing frequency-segmented deblurring (%2d/%2d)... ', idx, nr_recons, idx1, L);
        %------------------------------------------------------------------
        ...
        %------------------------------------------------------------------
        im_fs(:,:,idx1) = sum(bsxfun(@times, conj(csm), imc), 3);
        fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));
    end
    ...
    if L > 1
        ...
        for idx2 = 1:N2
            tstart = tic; fprintf('(%2d/%2d): Performing pixel-dependent interpolation (:,%3d/%3d)... ', idx, nr_recons, idx2, N2);
            for idx1 = 1:N1
    ...
    im_king_multislice(:,:,idx) = im_king;
end
end