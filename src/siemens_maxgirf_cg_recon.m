function [im_maxgirf_multislice,header,r_dcs_multislice,output] = siemens_maxgirf_cg_recon(ismrmrd_noise_fullpath, ismrmrd_data_fullpath, siemens_dat_fullpath, B0map, user_opts)
...
for idx = 1:nr_recons
    ...
    start_time_maxgirf = tic; fprintf('(%d/%d): Performing CG-based MaxGIRF reconstruction...\n', idx, nr_recons);
    E = @(x,tr) encoding_lowrank_maxgirf(x, csm, U(:,1:L,:), V(:,1:L,:), w, st, tr);
    [m_maxgirf, flag, relres, iter, resvec, lsvec] = lsqr(E, b, tol, max_iterations, [], [], []); % NL x 1
    im_maxgirf = reshape(m_maxgirf, [N1 N2]);
    computation_time_maxgirf = toc(start_time_maxgirf);
    fprintf('done! (%6.4f/%6.4f sec)\n', computation_time_maxgirf, toc(start_time));

    %% Collect the output
    im_maxgirf_multislice(:,:,idx,:) = im_maxgirf;
