function [im_maxgirf_multislice,header,r_dcs_multislice,output] = siemens_maxgirf_cg_recon_multi_gpu(ismrmrd_noise_fullpath, ismrmrd_data_fullpath, siemens_dat_fullpath, B0map, user_opts)
% Written by Nam Gyun Lee
% Email: namgyunl@usc.edu, ggang56@gmail.com (preferred)
% Started: 01/16/2022, Last modified: 01/25/2022
...
%--------------------------------------------------------------------------
% Set "support_constraint"
%--------------------------------------------------------------------------
if isfield(user_opts, 'support_constraint')
    support_constraint = user_opts.support_constraint;
else
...
% Transfer arrays from the CPU to the GPU 
%--------------------------------------------------------------------------
Ni_per_gpu = floor(Ni / nr_GPUs);

spmd (nr_GPUs)
    fprintf("Hi this is number %d/%d\n", labindex, numlabs);
    if labindex < numlabs
        interleaf_range = (1:Ni_per_gpu).' + (labindex - 1) * Ni_per_gpu;
    else
        interleaf_range = (1 + (labindex - 1) * Ni_per_gpu:Ni).';
    end
    nr_interleaves = length(interleaf_range);

    B0map_device = gpuArray(B0map);
    k_device = gpuArray(k(:,:,interleaf_range));
    t_device = gpuArray(t);
    w_device = gpuArray(w(:,interleaf_range));

    st_device = cell(nr_interleaves,1);
    for i = 1:nr_interleaves
        idx1 = interleaf_range(i);
        st_device{i}.n_shift = st{idx1}.n_shift;
        st_device{i}.alpha   = {gpuArray(st{idx1}.alpha{1}), gpuArray(st{idx1}.alpha{2})};
        st_device{i}.beta    = {gpuArray(st{idx1}.beta{1}), gpuArray(st{idx1}.beta{2})};
        st_device{i}.ktype   = st{idx1}.ktype;
        st_device{i}.tol     = st{idx1}.tol;
        st_device{i}.Jd      = st{idx1}.Jd;
        st_device{i}.Nd      = st{idx1}.Nd;
        st_device{i}.Kd      = st{idx1}.Kd;
        st_device{i}.M       = st{idx1}.M;
        st_device{i}.om      = gpuArray(st{idx1}.om);
        st_device{i}.sn      = gpuArray(st{idx1}.sn);
        st_device{i}.p       = gpuArray(st{idx1}.p);
    end
end
...
for idx = 1:nr_recons
    ...
    [m_sense, flag, relres, iter, resvec, lsvec] = lsqr(E, b, tol, max_iterations, [], [], []); % N x 1
    ...
    if support_constraint
        ...
        %------------------------------------------------------------------
        kspace_sense = bsxfun(@times, csm, im_sense);
        for dim = 1:2
            kspace_sense = 1 / sqrt(size(kspace_sense,dim)) * fftshift(fft(ifftshift(kspace_sense, dim), [], dim), dim);
        end
    ...
    %% Start parallel processing using multi-GPUs
    spmd (nr_GPUs)
        %% Transfer arrays from the CPU to the GPU
        kspace_device = gpuArray(kspace(:,interleaf_range,:));
        csm_device = gpuArray(csm);
        p_device = gpuArray(p);

        %% Calculate the SVD of the higher-order encoding matrix (Nk x N)
        U_device = complex(zeros(Nk, L, nr_interleaves, 'double', 'gpuArray'));
        V_device = complex(zeros(N, L, nr_interleaves, 'double', 'gpuArray'));
        ...
        b_device_composite = complex(zeros(N, 1, 'double', 'gpuArray'));
        for i = 1:nr_interleaves
            tstart = tic; fprintf('(%2d/%2d): Performing CP-based MaxGIRF reconstruction (i=%2d/%2d)... ', idx, nr_recons, interleaf_range(i), Ni);
            ...
            for c = 1:Nc
                ...
                % Calculate b (N x 1)
                %----------------------------------------------------------
                b_device_composite = b_device_composite + AHd;
            end
            fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));
        end
    end
    ...
    for idx1 = 1:nr_GPUs
        b_device = b_device + b_device_composite{idx1};
    end

    %% Perform CG-based MaxGIRF reconstruction
    start_time_maxgirf = tic; fprintf('(%2d/%2d): Performing CG-based MaxGIRF reconstruction...\n', idx, nr_recons);
    E = @(x,tr) encoding_lowrank_maxgirf_multi_gpu(x, csm_device, U_device, V_device, w_device, st_device, tr);
    ...
    output(idx).lsvec  = [];
    output(idx).computation_time_svd = [];
    output(idx).computation_time_cpr = [];    
