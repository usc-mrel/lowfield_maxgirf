function [im_cpr_multislice,header,r_dcs_multislice,output] = siemens_maxgirf_cp_recon_gpu(ismrmrd_noise_fullpath, ismrmrd_data_fullpath, siemens_dat_fullpath, B0map, user_opts)
...
for idx = 1:nr_recons
    ...
    start_time_cpr = tic;
    b_device = complex(zeros(N, Lmax, 'double', 'gpuArray'));
    for i = 1:Ni
        ...
        for c = 1:Nc
            ...
            %--------------------------------------------------------------
            AHd = complex(zeros(N, Lmax, 'double', 'gpuArray'));
            AHd_ = complex(zeros(N, 1, 'double', 'gpuArray'));
            ...
            for ell = 1:Lmax
                ...
                AHd_ = AHd_ + V_device(:,ell,i) .* reshape(FHDuHd, [N 1]);
                AHd(:,ell) = AHd_;
            end
            ...
            %--------------------------------------------------------------
            for ell = 1:Lmax
                AHd(:,ell) = reshape(conj(csm_device(:,:,c)), [N 1]) .* AHd(:,ell);
            end
            ...
            %--------------------------------------------------------------
            for ell = 1:Lmax
                b_device(:,ell) = b_device(:,ell) + AHd(:,ell);
            end
        end
        fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));
    end
    ...
    %% Collect the output
    im_cpr_multislice(:,:,idx,:) = reshape(gather(b_device), [N1 N2 1 Lmax]);
