function [im_sense_multislice,header,r_dcs_multislice,output] = siemens_sense_recon_gpu(ismrmrd_noise_fullpath, ismrmrd_data_fullpath, siemens_dat_fullpath, user_opts)
...
im_sense_multislice = complex(zeros(N1, N2, nr_slices, 'double'));
r_dcs_multislice = zeros(N, 3, nr_slices, 'double');
output = repmat(struct('flag', [], 'relres', [], 'iter', [], 'resvec', [], 'lsvec', []), [nr_recons 1]);
...
for idx = 1:nr_recons
    ...
    E = @(x,tr) encoding_sense_gpu(x, csm_device, w_device, nufft_st_device, tr);
    [m_sense_device, flag, relres, iter, resvec] = lsqr(E, b_device, tol, max_iterations, [], [], []); % N x 1
    im_sense_device = reshape(m_sense_device, [N1 N2]);
    fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));

    %% Collect the output
    im_sense_multislice(:,:,idx) = gather(im_sense_device);
